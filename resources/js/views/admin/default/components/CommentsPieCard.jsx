...
import {CommentsChartOptions } from "@/variables/admin/charts";
import Card from "@/components/admin/card";
import {getRandomColor} from "@/utils/randomColor";

const CommentsPieCard = (props) => {
    const {pieChartFooterParentCount, pieChartFooterChildCount } = props;
    const {title,footerParentTitle, footerChildTitle } = props;
    const arrayData = Object.values(props.pieChartData).filter(item => typeof item === 'object');
    CommentsChartOptions.labels = arrayData.map(item => item.category);
    const pieChartData = arrayData.map(item => item.count);
    CommentsChartOptions.colors = Array.from({length: CommentsChartOptions.labels.length}, () => getRandomColor());
    return (
        <Card extra="rounded-[20px] p-3">
            ...
            <div className="mb-auto flex h-[220px] w-full items-center justify-center mt-5">
                <PieChart options={CommentsChartOptions} series={pieChartData} />
            </div>
            <div className="flex flex-row !justify-between rounded-2xl px-6 py-3 shadow-2xl shadow-shadow-500 dark:!bg-navy-700 dark:shadow-none">
                ...
                <div className="flex flex-col items-center justify-center">
                    <div className="flex items-center justify-center">
                        <p className="ml-1 text-sm font-normal text-gray-600">Active/Inactive</p>
                    </div>
                    <p className="mt-px text-xl font-bold text-navy-700 dark:text-white">
...
export default CommentsPieCard;
